...
- When in the standing state the robot has a probability src_lua[:exports code]{Pw = math.max(PWmin, W - beta * N)} to change state
  into walking
- When in the walking state the robot has a probability src_lua[:exports code]{Ps = math.min(PSmax, S + alpha * N)} to change state
  into standing
